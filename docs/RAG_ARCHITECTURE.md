...
| `quoted_neutral`     | **drop** на выходе Stance Detector'а (с stance-v0.3) | ничего                        |

**Почему quoted_neutral тоже дроп** (правка после первого живого прогона
на ролике-разоблачении): в разоблачающих видео авторы регулярно цитируют
миф «учёные говорят X», чтобы тут же его разобрать. Модель честно ставит
такую цитату как `quoted_neutral`, потому что формально автор сам это
утверждение не делает. Если бы мы оставляли `quoted_neutral` в overlay,
расширение «исправляло» бы автора, который уже всё корректно рассказал.
В крайних случаях (легитимный обзор дискуссии) мы из-за этого пропустим
тезис — терпимая цена за отсутствие false-positives на разоблачающем
контенте.
